...
import com.typo3.fluid.linter.ui.fluida11y.UiStateTopic;
...
/**
 * Outline tab migrated from HeadingOutlinePanel. Shows headings for active file
 * with live unsaved preview when typing.
 */
public final class OutlineTab extends JPanel {
    ...
    private boolean unsavedPreviewEnabled;

    private VirtualFile currentFile;
    private Integer lastSelectedOffset;

    public OutlineTab(@NotNull Project project) {
        ...
        this.unsavedPreviewEnabled = project.getService(A11yUiState.class).isUnsavedPreviewEnabled();
        ...
        connection.subscribe(UnsavedOutlineTopic.TOPIC, (file, result) -> {
            if (!unsavedPreviewEnabled) return;
        ...
        connection.subscribe(UiStateTopic.TOPIC, state -> {
            unsavedPreviewEnabled = state.isUnsavedPreviewEnabled();
            // force refresh from saved if turning preview off
            if (!unsavedPreviewEnabled && currentFile != null) refreshFromSaved(currentFile);
        });
    ...
    private void restoreSelectionNear(int offset) {
        ...
        int bestIdx = 0; int bestDist = Integer.MAX_VALUE;
